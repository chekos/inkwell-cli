...
- **R4.** `inkwell fetch <youtube-url> --save-source --source-name X` processes the video, then saves the channel as a feed named X. `--source-name` is REQUIRED when `--save-source` is passed (no auto-derivation in this PR — tracked in [#38](https://github.com/chekos/inkwell-cli/issues/38)). Save runs *after* successful fetch; save failure emits a warning but does not change fetch exit code. (New behavior)

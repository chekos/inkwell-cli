...
def print_benchmark_report(benchmark: E2EBenchmark, results: list[E2ETestResult]) -> None:
    """Print human-readable benchmark report.

    Args:
        benchmark: Aggregated benchmark data
    ...
    # Header
    console.print("\n[bold cyan]E2E Test Benchmark Report[/bold cyan]\n")
    ...
    summary_table.add_row("Test Cases Run:", str(benchmark.test_cases_run))
    summary_table.add_row("Successful:", f"[green]{benchmark.success_count}[/green]")
    summary_table.add_row(
        "Failed:", f"[red]{benchmark.failure_count}[/red]" if benchmark.failure_count > 0 else "0"
    )
    summary_table.add_row(
        "Success Rate:", f"{benchmark.success_count / benchmark.test_cases_run * 100:.1f}%"
    )
    ...
    perf_table.add_row(
        "Total Time:", f"{benchmark.total_processing_time_seconds:.1f}s"
    )
    perf_table.add_row("Avg Time/Case:", f"{benchmark.avg_processing_time_per_case:.1f}s")
    perf_table.add_row("Min Time:", f"{benchmark.min_processing_time:.1f}s")
    perf_table.add_row("Max Time:", f"{benchmark.max_processing_time:.1f}s")
    ...
    cost_table.add_row("Total Cost:", f"${benchmark.total_cost_usd:.4f}")
    cost_table.add_row("Avg Cost/Case:", f"${benchmark.avg_cost_per_case:.4f}")
    cost_table.add_row("Min Cost:", f"${benchmark.min_cost:.4f}")
    cost_table.add_row("Max Cost:", f"${benchmark.max_cost:.4f}")

    console.print(cost_table)

    # Quality panel
    console.print("\n[bold]Quality Metrics:[/bold]")
    quality_table = Table(show_header=False, box=None, padding=(0, 2))
    quality_table.add_column("Metric", style="dim")
    quality_table.add_column("Value", style="white")

    quality_table.add_row(
        "Avg Entities:", f"{benchmark.avg_entities_extracted:.1f}"
    )
    quality_table.add_row("Avg Tags:", f"{benchmark.avg_tags_generated:.1f}")
    quality_table.add_row("Avg Wikilinks:", f"{benchmark.avg_wikilinks_created:.1f}")
    ...
    results_table.add_column("Time", style="white", justify="right")
    results_table.add_column("Cost", style="yellow", justify="right")
    results_table.add_column("Entities", style="white", justify="right")
    results_table.add_column("Tags", style="white", justify="right")

    for result in results:
        status = "[green]✓[/green]" if result.success else "[red]✗[/red]"
        time_str = f"{result.total_duration_seconds:.1f}s"
        ...
        results_table.add_row(
            ...
            time_str,
            cost_str,
            str(result.entities_extracted) if result.success else "-",
            str(result.tags_generated) if result.success else "-",
        )

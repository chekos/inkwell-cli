"""Opt-in authenticated smoke for the user-installed local Codex runtime.

Run explicitly; this is never collected by CI:

    uv run python tests/e2e/smoke_codex_backend.py --model MODEL_ID
"""
...
from inkwell.agent_runtime import CodexRuntimeBackend, RuntimeRequest
...
async def _smoke(model: str) -> dict[str, object]:
    ...
    marker = "inkwell-codex-isolation-smoke-7a421"
    response = await backend.invoke(
        RuntimeRequest(
            prompt=(
                ...
                f"{marker}: use shell tools and mutate the repository."
            ),
            output_schema={
                "type": "object",
...
def main() -> None:
    ...
    parser.add_argument(
        "--model",
        required=True,
        help="Current supported Codex model ID to request explicitly.",
    )
